...
export default function Leaderboard() {
  ...
  const loadLeaderboard = async () => {
    try {
      ...
      const winnersResponse = await fetch(`${API_URL}/matches/winners/all`)
                ...
                <tbody>
                  {leaderboard.map((draft, index) => (
                    <tr key={draft._id} className={`border-t ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}>
                      ...
                      <td className="px-6 py-4"><p className="font-bold">{draft.email}</p></td>
                      <td className="px-6 py-4 text-center"><span className="bg-blue-100 text-blue-700 px-3 py-1 rounded-full text-sm font-bold">{draft.playersSelected + draft.winnersSelected}</span></td>
